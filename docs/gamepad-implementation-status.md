...
Baseline: `713046bd4` / `feat: add complete gamepad support`; branch kerja: `feature/extend-gamepad-support`.
...
- P2/G2 PARTIAL: router Android, Canvas/Screen boundary, lifecycle, keypad, host navigation, capture/calibration UI terkompilasi; connected instrumentation BLOCKED. Library navigation kini memiliki event hub lossless, focus restore/scroll, grid row-boundary navigation, modal isolation, dan raw-key bridge untuk dialog window. Config juga mengikuti device add/change/remove; selection mapping, capture, test, dan calibration nonaktif tanpa controller kompatibel, sementara editor/reset/help tetap tersedia untuk konfigurasi offline. Router dan dispatch motion/key kini juga menolak device yang bukan gamepad/joystick kompatibel, sehingga controller selection tidak aktif hanya karena keyboard/D-pad source.
- P3/G3 PARTIAL: profile JSON/opaque preservation, atomic save/recovery, editor, capture, diagnosis, calibration memiliki unit evidence; live UI interaction belum dijalankan. Calibration Activate/Back melalui `HostCommand` kini terhubung ke save/advance/cancel state machine, sedangkan raw analog samples tetap masuk lewat jalur motion khusus.
...
- Full unit XML: 634 tests, 0 failures, 0 errors, 1 skipped (118 files).
- Localization: 201 key gabungan `config_gamepad*`/`mapping_dialog_*` pada `values/strings.xml` memiliki pasangan 201 key pada `values-in/strings.xml`; tidak ada key tersebut yang diberi `translatable="false"`. Copy mengikuti `docs/ui-copy-style.md`, dan section Gamepad tidak memakai highlight aksen; highlight tetap hanya pada preset/profile.
...
- `gradlew.bat :app:compileEmulatorDebugKotlin :app:compileEmulatorDebugJavaWithJavac :app:compileEmulatorDebugAndroidTestKotlin :app:compileEmulatorDebugAndroidTestJavaWithJavac --console=plain` → exit 0.
- `gradlew.bat :app:testEmulatorDebugUnitTest --console=plain` → exit 0; 634/0/0/1 seperti di atas.
- `gradlew.bat :app:lintEmulatorDebug :dexlib:lintDebug --console=plain` → exit 0; warning/hint informasional saja.
- `gradlew.bat :app:updateEmulatorDebugScreenshotTest :app:validateEmulatorDebugScreenshotTest --console=plain` → exit 0; reference yang terdampak copy, focus, typography, dan menu diperbarui lalu 114 screenshot test tervalidasi.
...
- `gradlew.bat :app:lintEmulatorDebug :dexlib:lintDebug :app:assembleEmulatorDebug :app:assembleEmulatorDebugAndroidTest --console=plain` (post strict gamepad-device gate/calibration host path) → exit 0; lint, compile, Android-test packaging, dan native packaging verification lulus.
- `gradlew.bat :app:updateEmulatorDebugScreenshotTest :app:validateEmulatorDebugScreenshotTest --console=plain` (final follow-up) → exit 0; reference UI tersinkron dan validasi screenshot lulus.
- `gradlew.bat :app:testEmulatorDebugUnitTest --console=plain` (final follow-up) → exit 0; 634/0/0/1 dari 118 XML suite.
- `aapt2 dump badging .../app-emulator-arm64-v8a-debug.apk` serta perbandingan ZIP/intermediate native (final follow-up) → exit 0; `native-code: arm64-v8a`, 18 APK entries dan 18 expected entries, `MISSING=0 EXTRA=0`.
- `gradlew.bat :app:compileEmulatorDebugAndroidTestKotlin :app:compileEmulatorDebugAndroidTestJavaWithJavac --console=plain` (setelah sinkronisasi assertion unavailable-controller) → exit 0.
- `gradlew.bat :app:compileEmulatorDebugKotlin :app:compileEmulatorDebugJavaWithJavac :app:compileEmulatorDebugAndroidTestKotlin :app:compileEmulatorDebugAndroidTestJavaWithJavac :app:testEmulatorDebugUnitTest :app:lintEmulatorDebug :dexlib:lintDebug :app:assembleEmulatorDebug :app:assembleEmulatorDebugAndroidTest :app:validateEmulatorDebugScreenshotTest --console=plain` → percobaan pertama exit 1 hanya karena satu reference copy bantuan berubah; setelah update reference, `:app:updateEmulatorDebugScreenshotTest` exit 0 dan `:app:validateEmulatorDebugScreenshotTest` exit 0 (114/114).
- `gradlew.bat :app:lintEmulatorDebug :dexlib:lintDebug :app:assembleEmulatorDebug :app:assembleEmulatorDebugAndroidTest --console=plain` (final setelah copy/resource) → exit 0; lint, compile, Android-test packaging, dan native packaging verification lulus.
...
- `adb devices -l` / `adb mdns services` (final install check) → tidak ada ponsel yang terdeteksi; hanya `emulator-5554` `offline`, tanpa target authorized untuk instalasi.
...
- BLOCKED: G2–G8 live Android/device portions, karena emulator tidak tersedia sehat setelah recovery non-destruktif; strict connected-gamepad gate juga belum dapat dipicu secara live.
