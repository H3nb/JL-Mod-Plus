...
public class PlatformCompatManifestTest {
	...
	@Test
	public void legacyActivityAliasesKeepExplicitComponentNamesResolvable() throws Exception {
		Context context = InstrumentationRegistry.getInstrumentation().getTargetContext();
		PackageManager packageManager = context.getPackageManager();
		assertLegacyAlias(packageManager, context, "ru.playsoftware.j2meloader.LauncherActivity",
				LauncherActivity.class.getName());
		assertLegacyAlias(packageManager, context, "ru.playsoftware.j2meloader.MainActivity",
				MainActivity.class.getName());
		assertLegacyAlias(packageManager, context, "ru.playsoftware.j2meloader.config.ConfigActivity",
				ConfigActivity.class.getName());
		assertLegacyAlias(packageManager, context, "ru.playsoftware.j2meloader.settings.SettingsActivity",
	...
	private static void assertLegacyAlias(PackageManager packageManager, Context context,
			String legacyName, String targetName) throws PackageManager.NameNotFoundException {
		ActivityInfo info = packageManager.getActivityInfo(new ComponentName(context, legacyName), 0);
		assertEquals(legacyName, info.name);
		assertEquals(targetName, info.targetActivity);
