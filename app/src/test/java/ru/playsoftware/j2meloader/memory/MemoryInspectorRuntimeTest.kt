...
class MemoryInspectorRuntimeTest {
    @Test fun inspectorCellsStayRelativeToTheCandidateAnchor() {
        ...
        val snapshot = MemoryInspectorSnapshot(
            candidateId = 9, type = MemoryEngineContract.TYPE_INT, label = "HP",
            startAddress = 0x1000, anchorAddress = 0x1004, bytes = bytes,
        )
        val cells = buildInspectorCells(snapshot)
    ...
    @Test fun inspectorFloatViewUsesLittleEndianSnapshotBytes() {
        ...
        val snapshot = MemoryInspectorSnapshot(
            candidateId = 2, type = MemoryEngineContract.TYPE_FLOAT, label = "Speed",
            startAddress = 0x3000, anchorAddress = 0x3000, bytes = bytes,
        )
        val cell = buildInspectorCells(snapshot).single()
