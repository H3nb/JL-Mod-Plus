...
public class NativeTombstoneSummaryTest {
	...
	@Test
	public void canonicalAndLegacyProjectFramesRemainActionable() {
		NativeTombstoneSummary.Summary summary = NativeTombstoneSummary.parse(
				tombstoneFixture(false, "io.github.h3nb.jlmodplus.micro3d.Utils.transform"));
		String text = NativeTombstoneSummary.format(summary);

		assertNotNull(text);
		assertTrue(text.contains("Top JL-Mod frame: io.github.h3nb.jlmodplus.micro3d.Utils.transform"));
	}

	private static byte[] tombstoneFixture(boolean truncateTail) {
		return tombstoneFixture(truncateTail, null);
	}

	private static byte[] tombstoneFixture(boolean truncateTail, String projectFrame) {
		...
		tombstone.message(16, threadEntry(19119, crashThread(projectFrame)));
	...
	private static byte[] crashThread(String projectFrame) {
		...
		String[] frames = {
				...
				projectFrame == null ? "javax.microedition.lcdui.Graphics.reset" : projectFrame,
				"DoCall",
				"ExecuteSwitchImplCpp",
				"ExecuteSwitchImplAsm",
